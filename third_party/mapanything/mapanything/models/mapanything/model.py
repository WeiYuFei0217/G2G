...
#
# Modifications (c) 2025 the G2G authors, relative to upstream v1.0.1:
#   `MapAnything.forward()` gains an optional `return_info_sharing_features`
#   argument (default False). When True, forward() early-returns the
#   multi-view information-sharing features (before the DPT head) for the
#   downstream G2G pose-estimation module. See ../../NOTICE for details.
...
class MapAnything(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, views, memory_efficient_inference=False, return_info_sharing_features=False):
        """
        ...
        Args:
            ...
            return_info_sharing_features (bool): If True, return info_sharing features (before DPT head) instead of final outputs.
                                                  Useful for downstream tasks like G2G pose estimation. Default is False.

        Returns:
            If return_info_sharing_features is False:
            List[dict]: A list containing the final outputs for all N views.
            If return_info_sharing_features is True:
                List[torch.Tensor]: A list of tensors [B, C, H_patch, W_patch] for each view, containing info_sharing output features.
        """
        ...
        # Early return for G2G and other downstream tasks that need info_sharing features
        if return_info_sharing_features:
            # Return list of [B, C, H_patch, W_patch] tensors for each view
            return final_info_sharing_multi_view_feat.features
