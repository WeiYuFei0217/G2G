#!/bin/bash
# Multi-GPU parallel precomputation of DINOv2 features (val set)
# Each GPU acts as one worker, sharding scenes by modulo for parallel processing

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
STEP3_ROOT="/path/to/data/HM3D/DATA_GEN/step3_render_224_224_uint16_val_2_5ins"
OUTPUT_ROOT="/path/to/data/HM3D/DATA_GEN/step6_dinov2_features_224_val_2_5ins"
MODEL_PATH="/path/to/map-anything-model/"
LOG_DIR="${SCRIPT_DIR}/logs_step6_val"
...
EXTRA_ARGS="--skip-existing"  # Support resume / skip-existing
# ==================================================

NUM_WORKERS=${#GPUS[@]}

echo "=== Step 6: Multi-GPU parallel precomputation of DINOv2 features (val) ==="
